...
using (var client = new HttpClient()) {
    ...
    foreach ( var m in myMeasures)
    {
        ...
        "{" +
        "\"role\": \"system\"," +
        "\"content\": \"You are Power BI and business expert. The underlying dataset contains Spaceparts datas. Spaceparts is a company that sells spaceship parts across the universe. Some business insight: * Currency conversion in multiple rates * Trends that differ by region, customer, and product groups * Hierarchies for regions, customers, and products * Personally identifiable information for the (fictional) Spaceparts employees. Your task is to explain the DAX calculation in a few sentences in simple business terms without using DAX function names. It need to understand people who are not familiar with the data model and DAX." + 
        "Please give suggestion block with bulletpoints where a Spacepart business user can use the affected measurement. It is important to ALWAYS stay in Spaceparts context while you are giving suggestions. Please don't try to decipher abbreviations. Please don't try to explain the exact calculation method (Don't use like this: this calculation adds the number to a constant value of zero), only the meaning of calculation! (ex.: measure give back 0 if no orders)\"" +
        "}," +
        "{" +
        "\"role\": \"user\"," +
