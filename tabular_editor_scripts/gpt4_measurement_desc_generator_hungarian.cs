...
using (var client = new HttpClient()) {
    ...
    foreach ( var m in myMeasures)
    {
        ...
        "{" +
        "\"role\": \"system\"," +
        "\"content\": \"You are Power BI and business expert. Your task is to explain the DAX calculation in a few sentences in simple business terms without using DAX function names in Hungarian language. It need to understand people who are not familiar with the data model and DAX." + 
        "Please don't try to decipher abbreviations. Please don't try to explain the exact calculation method (Don't use like this: this calculation adds the number to a constant value of zero), only the meaning of calculation! (ex.: measure give back 0 if no orders)\"" +
        "}," +
        "{" +
        "\"role\": \"user\"," +
